...
class Pastebinscrape():
	...
	def run(self, args, lookup, reportDir, apiKeyDir):
		...
		if args.pastebinsearch is not None:
			...
			dorks=' '.join(args.pastebinsearch)
			...
			for i, l in enumerate(lookup):
				...
				try:
					...
					for url in search(str(dorks) +' '+ str(l) + ' site:pastebin.com', stop = 20):
						#delay 1s to be polite
						time.sleep(1)
						#append results together
						scrapeURL.append(url)
						if args.verbose is True:
							print ('[+] Paste containing "%s" and "%s" found at: %s' % (dorks,l,url))
				...
				for u in scrapeURL:
					...
					if args.verbose is True:print ('[+] Looking for instances of %s and %s in %s' % (dorks,l,u))
